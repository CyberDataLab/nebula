...
async def physical_nodes_available(ips: list[str]) -> bool:
    ...
    for st in states:
        if not isinstance(st, dict):
            return False               #  excepción ⇒ nodo caído
        if st.get("running") is True:  #  entrenando ⇒ ocupado
            return False
    return True
...
async def run_scenario(scenario_data: dict, role: str, user: str) -> None:
    ...
    - If physical: wait until all Raspberry Pi nodes are idle.
    - Then reserve GPU/CPU depending on availability and role.
    - Call the controller to launch the scenario.
    - Register the necessary information to manage node completion.
    """
    user_data = user_data_store[user]
    ...
    if is_physical:
        ...
        while not await physical_nodes_available(phys_ips):
            ...
            logging.info(
                ...
                f"Raspberry Pis are busy - waited for {mins:02d}:{secs:02d}; "
                "retrying in 10 seconds"
            )
            await asyncio.sleep(10)

    # Assign CPU/GPU based on availability
    scenario_data = await assign_available_gpu(scenario_data, role)

    # Launch the scenario using the controller
    scenario_name = await deploy_scenario(scenario_data, role, user)

    # Register information for node synchronization
